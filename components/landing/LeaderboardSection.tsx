...
/** Mobile card rank accents (screenshot design). */
function mobileRankClass(rank: number): string {
  if (rank === 1) return "text-amber-400";
  if (rank === 2) return "text-sky-400";
  if (rank === 3) return "text-orange-500";
  if (rank === 4) return "text-indigo-400";
  if (rank === 5) return "text-sky-300";
  return "";
}
  ...
  return (
    <div className={`rounded-xl border-l-4 ${r.rowBorder} ${cardShell}`}>
      <div className={ROW_GRID}>
        ...
        <span
          className={`min-w-0 truncate font-semibold ${isLight ? "font-bold text-slate-900" : "text-bqa-text"}`}
        >
          {r.city}
        </span>
        <span
        ...
        {!loading && !error && (
          <>
            ...
            <div className="hidden md:block">
              <div className={`${ROW_GRID} ${headerClass}`}>
                <span>Rank</span>
                <span>City</span>
